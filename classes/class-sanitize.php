...
class Formidable_Schedule_Export_Sanitize {
	...
	public function clean( $data, $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'formidable-schedule-export' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'formidable-schedule-export' ) );
		 ...
		 */
		do_action( 'formidable_schedule_export_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'formidable_schedule_export_post_sanitize', $sanitized );
